...
describe('Kanban Board', () => {
  it(
    ...
    () => {
      ...
      expect(board).toEqual(expect.objectContaining({ title: expect.stringMatching(""), statuses: expect.arrayContaining([])}))
    },
  );
  ...
  it('have a URL property that has the title in kebab case', () => {
    ...
    expect(board).toEqual(expect.objectContaining({ url: expect.stringContaining('important-things')}))
  });
});
