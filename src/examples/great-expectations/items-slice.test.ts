...
it('supports adding an item with the correct name', () => {
  ...
  expect(result).toEqual(expect.arrayContaining([ expect.objectContaining({ name: 'iPhone' })]))
});

it('prefixes ids with "item-"', () => {
  expect.hasAssertions();
  const result = reducer([], add({ name: 'iPhone' }));
  expect(result).toEqual(expect.arrayContaining([ expect.objectContaining({ id: expect.stringMatching(/^item-/)})]))
});

it('defaults new items to a packed status of false', () => {
  expect.hasAssertions();
  const result = reducer([], add({ name: 'iPhone' }));
  expect(result).toEqual(expect.arrayContaining([expect.objectContaining({ packed: false})]))
});
